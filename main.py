...
def build_watermark_keyboard(setup: dict) -> InlineKeyboardMarkup:
    ...
    start_label = "📥 START DOWNLOAD 📥" if mode == "download" else "⚡ START RECORDING ⚡"
    rows.append([InlineKeyboardButton(start_label, callback_data="wm_start")])
...
async def rec_command(client, message: Message):
    ...
    msg = await message.reply_text("🔍 **Detecting audio tracks... Please wait.**")
    tracks = await detect_audio_tracks(url)
    ...
    if tracks:
        text = (
            f"🎵 **Video Selected.** Now select audio tracks for **{raw_filename}**.\n\n"
            f"🔗 URL: `{url[:60]}...`\n⏱ Duration: `{timestamp}`"
        )
        kb = build_audio_keyboard(tracks, selected, mode="record")
    else:
        text = setup_summary_text(user_setup[user_id])
    ...
    try:
        ...
        if auto_mode:
            ...
            cmd1 = (
                ...
                f'-i "{url}" -map 0:v {audio_maps} {vf} '
                f'{video_codec} -c:a copy -t 00:01:00 "{part1}"'
            )
            ...
            if job_key not in cancelled_jobs:
                ...
                cmd2 = (
                    f'ffmpeg -y -probesize 10000000 -analyzeduration 15000000 '
                    f'-ss {seek_to} -i "{url}" -map 0:v {audio_maps} {vf} '
                    f'{video_codec} -c:a copy -t 00:01:00 "{part2}"'
                )
            ...
            ffmpeg_cmd = (
                f'ffmpeg -y -probesize 10000000 -analyzeduration 15000000 '
                f'-i "{url}" -map 0:v {audio_maps} {vf} '
                f'{video_codec} -c:a copy {time_arg} "{video_path}"'
            )
