...
active_recordings = set()  # Track users with active recordings

# 🔧 RECORDING LIMIT - Maximum 3 concurrent recordings
...
async def rec_command(client, message: Message):
    ...
    if len(active_recordings) >= MAX_CONCURRENT_RECORDINGS:
        ...
        return await message.reply_text(
            ...
            f"🛑 Please wait for an existing recording to complete.\n"
            f"📱 Or use /cancel to stop a recording.",
            parse_mode="markdown"
        )
        ...
        async def update_recording_progress():
            while user_id in user_tasks:
                ...
                progress_text = (
                    ...
                    f"📊 **Active:** `{len(active_recordings)}/{MAX_CONCURRENT_RECORDINGS}`\n\n"
                    f"🛑 Use /cancel to stop recording"
                )
